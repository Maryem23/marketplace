...
$(function() {
	...
	if($productsTable.length) {
		...
		$productsTable.DataTable({
					lengthMenu : [ [ 10, 30, 50, -1 ], [ '10', '30', '50', 'TOUS' ] ],
					pageLength : 30,
					ajax : {
						url : jsonUrl,
						dataSrc : ''
					...
					initComplete: function () {
						...
						api.$('.switch input[type="checkbox"]').on('change' , function() {							
							var dText = (this.checked)? 'Vous souhaitez rendre le produit visible ?': 'Vous souhaitez rendre le produit invisible ?';
		...
		$categoryForm.validate({			
				...
				messages: {					
					name: {
						...
						minlength: 'Veuillez saisir au minimum cinq caractères'
					},
					description: {
						required: 'Veuillez saisir le nom du produit',
						minlength: 'Veuillez saisir au minimum cinq caractères'
		...
		if(countField.val() !== originalCount) {	
			...
			if(countField.val() < 1 || countField.val() > 3) {
				...
				bootbox.alert({
					size: 'medium',
			    	title: 'Erreur',
			    	message: 'La quantité du produit doit être au minimum 1 et au maximum 3 !'
				});
			}
			else {
